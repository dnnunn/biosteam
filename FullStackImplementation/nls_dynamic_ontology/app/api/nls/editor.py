\
    ...
    def replace_unit(sc: Scenario, template_or_id_from: str, template_to: str) -> List[dict]:
        try:
            idx = _unit_index(sc, template_or_id_from)
        except KeyError:
            idx = _find_first_by_template(sc, template_or_id_from)
            if idx is None:
    ...
    def set_params(sc: Scenario, params: Dict[str, Any], scope: str | None) -> List[dict]:
        ...
        if scope:
            try:
                targets = [_unit_index(sc, scope)]
            except KeyError:
                idx = _find_first_by_template(sc, scope)
                if idx is None:
                    raise KeyError(f"Scope not found: {scope}")
                targets = [idx]
        else:
            targets = [0] if sc.units else []
        for idx in targets:
    ...
    def add_unit(sc: Scenario, unit_template: str, after: str | None, before: str | None, at: str | None) -> List[dict]:
        ...
        new_id = _ensure_unit_id(sc, base)
        patches: List[dict] = []
        patches.append({"op": "add", "path": f"/units/-", "value": {"template": unit_template, "id": new_id, "overrides": {}}})
        if after:
            out_links = [sc.streams[i] for i in _find_links(sc, src=after)]
            for link in out_links:
                ridx = _find_links(sc, src=after, dst=link.to)[0]
            ...
            in_links = [sc.streams[i] for i in _find_links(sc, dst=before)]
            for link in in_links:
                ridx = _find_links(sc, src=link.from_, dst=before)[0]
                patches.append({"op": "remove", "path": f"/streams/{ridx}"})
    ...
    def remove_unit(sc: Scenario, target: str) -> List[dict]:
        try:
            idx = _unit_index(sc, target)
        except KeyError:
            idx = _find_first_by_template(sc, target)
            if idx is None:
                raise KeyError(f"Unit not found: {target}")
        unit_id = sc.units[idx].id
        patches: List[dict] = []
        for i in sorted(_find_links(sc, src=unit_id) + _find_links(sc, dst=unit_id), reverse=True):
